...
async def upload_file(request: Request, file: UploadFile = File(...)):
    ...
    if file.filename.endswith(".pdf") and contents[:4] != b'%PDF':
...
async def analyze(request: Request, session_id: str, query: str = "Bu ay nasıl harcadım?"):
    ...
    result_data = {
        ...
        "inflation_analysis": result.get("inflation_analysis") or {},
        "tax_breakdown": result.get("tax_breakdown") or {},
        "fx_shield": result.get("fx_shield") or {},
        "proactive_alerts": result.get("proactive_alerts") or [],
        "parsed_summary": result.get("parsed_summary") or {},
        "final_response": result.get("final_response") or "",
        "transactions": result.get("transactions") or [],
        "health_score": result.get("health_score") or {},
        "awareness_message": result.get("awareness_message") or "",
    ...
    session_store[session_id]["result"] = result_data
...
async def query(request: Request, request_body: QueryRequest):
    ...
    clean_query = sanitize_query(request_body.query)
...
async def save_profile(request: Request, profile_req: FinancialProfileRequest):
    ...
    profile = {
        ...
        "yatirim_listesi": profile_req.yatirim_listesi,
    ...
    if cached and saved:
        ...
        if parsed.get("tur") == "gider":
            mevcut_summary["toplam_gider"] = (
                (mevcut_summary.get("toplam_gider") or 0) + abs(parsed.get("tutar", 0))
            )
        else:
            mevcut_summary["toplam_gelir"] = (
                (mevcut_summary.get("toplam_gelir") or 0) + abs(parsed.get("tutar", 0))
            )
